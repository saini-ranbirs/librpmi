...
/** RPMI MM service types */
enum rpmi_mm_service_type {
	RPMI_MM_SERVICE_EFI,
	RPMI_MM_SERVICE_MAX
};

/** Platform specific MM EFI operations */
...
struct rpmi_mm {
	...
	/** MM service type */
	enum rpmi_mm_service_type	svc_type;

	union {
		/** Details required by MM EFI service */
		struct rpmi_mm_efi {
		...
		/**
		 * Nothing for MM non-EFI service at present. So, this is just
		 * a placeholder for any new MM service type addition in future,
